...
import { useGetUserById } from "@/modules/user/api/useGetUserById";
...
export default function UserProfilePage({ params }: UserPageProps) {
  ...
  // Reusing the feed mocks for the user's history
  const { data: posts, isLoading: postsLoading } = useGetPosts();
  const { data: comments, isLoading: commentsLoading } = useGetComments("mock-post-id");
